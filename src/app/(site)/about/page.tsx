...
import { getAboutContent, getMedia, getSettings, siteUrl } from '@/lib/content';
...
export const metadata: Metadata = {
  ...
  openGraph: { title: 'About Souradeep Sinha Photography' },
};
    ...
    <div className="bg-ivory">
      <section className="mx-auto grid w-full max-w-7xl gap-10 px-5 pb-20 pt-32 sm:px-8 sm:pt-40 md:grid-cols-[1.05fr_0.95fr] md:gap-16">
        <div>
          ...
          <h1 className="mt-4 font-display text-4xl leading-tight sm:text-6xl">{about.heading}</h1>
        ...
        {lead ? (
          <figure className="aspect-[4/5] w-full overflow-hidden bg-ink/5">
            ...
            <img
              ...
              alt={lead.alt || `${settings.studio_name} — photographer portrait`}
              className="hero-img"
              style={{ ['--focal' as string]: lead.focal || '50% 38%' }}
              fetchPriority="high"
              decoding="async"
      ...
      <section className="mx-auto w-full max-w-3xl px-5 pb-20 sm:px-8">
        <div className="whitespace-pre-line text-lg leading-relaxed text-ink/75">{about.body}</div>
      </section>
            ...
            {rest.map((image, index) => (
              <figure
                ...
                className={`w-full overflow-hidden bg-ink/5 ${
                  index % 3 === 1 ? 'aspect-[4/5] sm:mt-12' : 'aspect-[3/4]'
                }`}
              >
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img
      ...
      <section className="mx-auto w-full max-w-3xl px-5 pb-28 sm:px-8">
        {about.closing ? (
        ...
        <div className="mt-10 flex flex-wrap gap-4 text-[11px] uppercase tracking-[0.3em]">
          ...
          <Link href="/portfolio" className="px-4 py-3 text-ink/70 hover:text-gold">
            See the weddings
          </Link>
        </div>
      </section>
